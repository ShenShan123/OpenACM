...
def load_global_config(file_path: str) -> GlobalConfig:
    ...
    try:
        ...
        if "sram_compiler" in config_path.parts:
            base_root = Path(*config_path.parts[: config_path.parts.index("sram_compiler")])
        else:
            base_root = config_path.parent
        for key in ["pdk_path_TT", "pdk_path_FF", "pdk_path_SS", "pdk_path_FS", "pdk_path_SF"]:
            value = config_data.get(key)
            if value:
                path = Path(value)
                if not path.is_absolute():
                    config_data[key] = str(base_root / path)
...
class Parameter:
    ...
    def __init__(self, name: str, param_type: str, instance_names: Union[str, List[str]], description: str, value: Any, upper: Optional[Union[float, List[float]]] = None, lower: Optional[Union[float, List[float]]] = None, choices: Optional[List[str]] = None):
        self.name = name
    ...
    def _validate_types(self):
        ...
        if self.upper is not None:
            if "continuous" in self.param_type:
                if "scalar" in self.param_type:
                    # print(type(self.upper))
                    # print(type(self.lower))
                    # print(self.upper)
                    # print(self.lower)
                    # exit(0)
    ...
    def __repr__(self):
        return f"<Parameter {self.name} ({self.param_type}): " f"instances={self.instance_names}, value={self.value}>"
    ...
    def load_config(self, file_path: str, config_name: str) -> CircuitConfig:
        ...
        try:
            ...
            for param_name, param_data in circuit_config.get("parameters", {}).items():
                param = Parameter(name=param_name, param_type=param_data["type"], instance_names=param_data["names"], description=param_data["description"], value=param_data["value"], upper=param_data.get("upper"), lower=param_data.get("lower"), choices=param_data.get("choices"))
...
class SRAM_CONFIG:
    ...
    def __repr__(self):
        """返回配置的简洁摘要"""
        configs = [f"全局配置: {self.global_config is not None}", f"SRAM单元: {self.sram_6t_cell is not None}", f"字线驱动器: {self.wordline_driver is not None}", f"预充电: {self.precharge is not None}", f"列选择器: {self.column_mux is not None}", f"灵敏放大器: {self.senseamp is not None}", f"写入驱动器: {self.write_driver is not None}", f"译码器: {self.decoder is not None}"]
        return "SRAM配置状态:\n  " + "\n  ".join(configs)
...
if __name__ == "__main__":
    ...
    circuit_files = {"SRAM_6T_CELL": "yaml/sram_6t_cell.yaml", "WORDLINEDRIVER": "yaml/wordline_driver.yaml", "PRECHARGE": "yaml/precharge.yaml", "COLUMNMUX": "yaml/mux.yaml", "SENSEAMP": "yaml/sa.yaml", "WRITEDRIVER": "yaml/write_driver.yaml", "DECODER": "yaml/decoder.yaml"}
