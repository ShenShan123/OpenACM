...
class BaseSubcircuit(SubCircuit):
    ...
    def __init__(self, #接受晶体管参数和rc参数
        ...
        self.pi_res = pi_res
        self.pi_cap = pi_cap
        # 规范化参数文件路径为基于 sram_compiler 根目录的绝对路径
    ...
    def _normalize_param_file_path(self, param_file):
        ...
        将传入的参数文件路径规范化为绝对路径：
        - 若为绝对路径，直接返回
        - 若以 "sram_compiler" 开头，基于代码所在的 sram_compiler 目录解析
        - 否则，默认在 sram_compiler 目录下查找相对路径
        - 若上述路径不存在，回退到项目根目录 + 原始相对路径（适配不同工作目录）
        """
        pf = Path(param_file)
