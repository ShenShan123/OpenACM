...
class Sram6TCoreTestbench(BaseTestbench):  # sram阵列测试平台，继承自BaseTestbench
    def __init__(self, sram_config, w_rc=False, pi_res=10 @ u_Ohm, pi_cap=0.001 @ u_pF, custom_mc: bool = False, param_sweep: bool = False, sweep_precharge: bool = False, sweep_senseamp: bool = False, sweep_wordlinedriver: bool = False, sweep_columnmux: bool = False, sweep_writedriver: bool = False, sweep_decoder: bool = False, corner="TT", choose_columnmux: bool = True, q_init_val: int = 0, gen_unused_cells: bool = False):
        ...
        self.gen_unused_cells = gen_unused_cells  # 是否生成未使用单元
        # default mux inputs
        self.mux_in = 1
        # self.set_vdd(5)
    ...
    def create_decoder(self, circuit: Circuit):
        ...
        # 实例化译码器
        circuit.X("DECODER", decoder.NAME, self.power_node, self.gnd_node, *address_nodes, *wl_nodes)  # VDD, VSS  # 地址信号  # 字线输出
        # 保存译码器输出节点供字线驱动器使用
    ...
    def create_single_cell_for_snm(self, circuit: Circuit, operation: str):
        ...
        if self.custom_mc:
            ...
            )  # NOTE: Key argument to disconnect the internal data nodes!!
    ...
    def create_testbench(self, operation, target_row, target_col):
        ...
        if self.custom_mc:
            sbckt_6t_array = Sram6TCoreForYield(
                ...
                config=self.sram_config.global_config,
            )
        else:
        ...
        for bit in range(n_bits):
            ...
            node_name = f"A{bit}"
            if bit_val:
                circuit.PulseVoltageSource(f"ADDR_{bit}", node_name, self.gnd_node, initial_value=0 @ u_V, pulsed_value=self.vdd @ u_V, delay_time=5.0e-9, rise_time=self.t_rise, fall_time=self.t_fall, pulse_width=7.5e-9, period=self.t_period)  # 预充电开始后5ns  # 保持有效
            else:
                circuit.V(f"ADDR_{bit}", node_name, self.gnd_node, 0 @ u_V)
