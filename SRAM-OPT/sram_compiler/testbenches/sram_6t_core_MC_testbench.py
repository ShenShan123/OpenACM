...
class Sram6TCoreMcTestbench(Sram6TCoreTestbench):
    def __init__(self, sram_config, w_rc=False, pi_res=10 @ u_Ohm, pi_cap=0.001 @ u_pF, vth_std=0.05, use_mc=False, custom_mc=False, param_sweep=False, sweep_precharge=False, sweep_senseamp=True, sweep_wordlinedriver=False, sweep_columnmux=False, sweep_writedriver=False, sweep_decoder=False, coner="TT", choose_columnmux=True, q_init_val=0, sim_path="sim", gen_unused_cells=False):
        ...
        参数:
            ...
            q_init_val: 初始Q值
            sim_path: 仿真结果保存路径
            gen_unused_cells: 是否生成未使用的单元（为False时使用等效sram cell）
        """
        super().__init__(sram_config, w_rc, pi_res, pi_cap, (custom_mc and use_mc), param_sweep, sweep_precharge, sweep_senseamp, sweep_wordlinedriver, sweep_columnmux, sweep_writedriver, sweep_decoder, coner, choose_columnmux, q_init_val, gen_unused_cells)  # 父类
        self.use_mc = use_mc
    ...
    def add_meas_and_print(self, simulator, init_cond, operation):
        ...
        # The read operation
            ...
            # )
            simulator.measure("TRAN", "PSTC", f"AVG {{V(VDD)*I(VVDD)}} FROM={float(8.0 @ u_ns)} TO={float(10.0 @ u_ns)}")
            ...
            # simulator.circuit.raw_spice += f".PRINT TRAN FORMAT=NOINDEX " + "".join([f"V(WL{row}) " for row in range(self.num_rows)]) + "\n"
        # The write operation
    ...
    def add_xyce_options(self, circuit, mc_runs, operation):
        ...
        circuit.raw_spice += ".OPTIONS TIMEINT RELTOL=1e-4 ABSTOL=1e-12\n"
        circuit.raw_spice += ".OPTIONS NONLIN ITL4=200\n"
        if self.use_mc:
            circuit.raw_spice += f".OPTIONS SAMPLES numsamples={mc_runs}\n"
    ...
    def run_mc_simulation(self, operation="read", target_row=0, target_col=0, mc_runs=100, temperature=27, vars=None):
        ...
        simulator = circuit.simulator(temperature=temperature, nominal_temperature=27)  # 通过 **kwargs 传递  # 通过 **kwargs 传递
        ...
        self.add_xyce_options(simulator.circuit, mc_runs, operation)
        ...
        try:
            ...
            # important 改速率需要修改这里的倍数，现在是动态功耗分散到14ns到仿真时长里了，而实际的主频是200M
            if operation == "write":
