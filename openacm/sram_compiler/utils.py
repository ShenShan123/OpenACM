...
# gen_unused_cells = False


def parse_mc_measurements(netlist_prefix: str = "simulation", file_suffix: str = "mt", num_runs: int = 100, missing_value: float = np.nan, value_threshold: float = 1e-30) -> pd.DataFrame:
    """
...
def save_mc_results(df: pd.DataFrame, stats_df: pd.DataFrame, data_file: str = "mc_results.csv", stats_file: str = "mc_statistics.csv") -> None:
    """
...
def read_prn_with_preprocess(prn_file_path):
    ...
    try:
        ...
        df = pd.read_csv(prn_file_path, sep="\s+", skiprows=1, header=None, names=headers, engine="python", dtype=np.float64, comment="E", on_bad_lines="warn")
...
def split_blocks(df, analysis_type, num_mc):
    ...
    if analysis_type == "tran":
        ...
        if auto_block_count != num_mc:
            print(f"Warning: Auto-split block count ({auto_block_count}) does not match specified num_mc ({num_mc})")
            print(f"Using actual block count ({auto_block_count}) for processing")
            # Update num_mc to match actual block count for consistency
            num_mc = auto_block_count
...
def visualize_results(blocks, analysis_type, output_file):
    ...
    #     'figure.figsize': [8, 8],         # Figure size
    #     'figure.dpi': 350,                # Resolution (DPI)
    # })
    ...
    for color, signal in zip(colors, signals):
        ...
        for blk_idx, blk in enumerate(blocks):
            try:
                ...
                # Plot raw trajectory
                ax.plot(x, y, color=color, alpha=LINE_ALPHA, linewidth=LINE_WIDTH, zorder=1)
        ...
        if len(valid_samples) > 10:
            ...
            for x, y in valid_samples[:10]:
                ax.plot(x, y, color=color, alpha=0.3, linewidth=1, zorder=2)
    ...
    legend_elements = [Line2D([0], [0], color=c, lw=2, label=s) for s, c in zip(signals, colors)]
    ...
    for section in model_sections[1:]:
        if section.strip():
            ...
            if len(parts) >= 2:
                ...
                models[model_name] = {"name": model_name, "type": model_type, "parameters": parameters}
