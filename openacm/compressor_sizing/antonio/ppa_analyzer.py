#!/usr/bin/env python3
"""
PPA Analysis Tool for Antonio 4-2 Compressor
Version 1.0
usage: Xyce antonio.sp => python3 ppa_analyzer.py -m antonio.sp.mt0 -n antonio.sp
"""
...
class PPAAnalyzerAntonio:
    """Analyzes PPA metrics from Xyce simulation outputs for antonio"""
    
    # Default area values (used when extraction from netlist fails)
    DEFAULT_AREA_METRICS = AreaMetrics(
        total_area=0.9960,  # um^2 (based on typical antonio design)
        nmos_area=0.3980,  # um^2
        pmos_area=0.5980,  # um^2
        transistor_count=48,
        nmos_count=24,
        pmos_count=24,
    ...
    def __init__(self, measure_file: str = "antonio.sp.mt0", netlist_file: str = None):
        ...
        self.ppa_report = {
            ...
            'metadata': {
                ...
                'design': 'antonio',
                'architecture': '1×OR2 + 1×AOI21 + 1×INV + 2×NAND2 + 2×XNOR2 + 1×OAI21',
            }
        }
        self.report_lines = []
    ...
    def analyze_performance(self):
        """Extract and analyze performance metrics for antonio"""
        ...
        delay_patterns_s = [
            ...
            ('TPHL_C_S', 'C→S (P2)', 'Fall'),
            ('TPLH_B_S', 'B→S (P3)', 'Rise'),
            ('TPHL_BCD_S', 'BCD→S (P4)', 'Fall'),
        ]
        ...
        for key, label, edge in delay_patterns_s:
            if key in self.measurements:
                ...
                if 0 < val < 1e9:
                    delays_s.append(val)
                    self.add_report_line(f"    {label:14s} {edge:4s}: {val:7.2f} ps")
        
        # C output delays
        self.add_report_line("\n  Carry Output (C):")
        
        delay_patterns_c = [
            ('TPLH_C_C', 'C→C (P2)', 'Rise'),
            ('TPHL_BCD_C', 'BCD→C (P4)', 'Fall'),
        ]
        
        for key, label, edge in delay_patterns_c:
            if key in self.measurements:
                val = self.measurements[key] * 1e12
                if 0 < val < 1e9:
                    delays_c.append(val)
        ...
        if 'AVG_DELAY_C' in self.measurements:
        ...
        if 'RISE_S' in self.measurements:
            rise_s = self.measurements['RISE_S'] * 1e12
            if 0 < rise_s < 1e6:
                perf['rise_time_sum'] = rise_s
                self.add_report_line(f"  Sum (S) rise time:       {rise_s:7.2f} ps")
        
        if 'FALL_S' in self.measurements:
            fall_s = self.measurements['FALL_S'] * 1e12
            if 0 < fall_s < 1e6:
                perf['fall_time_sum'] = fall_s
                self.add_report_line(f"  Sum (S) fall time:       {fall_s:7.2f} ps")
            
        if 'RISE_C' in self.measurements:
            rise_c = self.measurements['RISE_C'] * 1e12
            if 0 < rise_c < 1e6:
                perf['rise_time_carry'] = rise_c
                self.add_report_line(f"  Carry (C) rise time:     {rise_c:7.2f} ps")
                
        if 'FALL_C' in self.measurements:
            fall_c = self.measurements['FALL_C'] * 1e12
            if 0 < fall_c < 1e6:
                perf['fall_time_carry'] = fall_c
                self.add_report_line(f"  Carry (C) fall time:     {fall_c:7.2f} ps")
    ...
    def analyze_power(self):
        ...
        if 'P_TOTAL_AVG' in self.measurements:
        ...
        if 'E_PER_OP' in self.measurements:
        ...
        elif 'E_TOTAL' in self.measurements:
            # Antonio has 4 operations
            e_per_op = e_total / 4.0
            power['energy_per_op'] = e_per_op
            self.add_report_line(f"  Energy per operation:    {e_per_op:8.2f} fJ")
        ...
        if p_dyn_avg > 0 and p_static_avg > 0:
            ...
            if total_components > 0:
                ...
                # If P_TOTAL_AVG differs significantly from component sum, add note
                if 'P_TOTAL_AVG' in self.measurements:
                    p_total = self.measurements['P_TOTAL_AVG'] * 1e6
                    ...
                    if diff_percent > 5.0:
                        ...
                        self.add_report_line(f"        Components differ from P_TOTAL_AVG by {diff_percent:.1f}%")
    ...
    def analyze_area(self):
        """Calculate area metrics from transistor sizes for antonio"""
        self.add_report_line("\n" + "="*80)
    ...
    def save_report_txt(self, output_file: str = "antonio_PPA_report.txt"):
    ...
    def save_report_json(self, output_file: str = "antonio_PPA_report.json"):
    ...
    def run_analysis(self):
        ...
        self.add_report_line("="*80)
        self.add_report_line(f"ANTONIO - PPA ANALYSIS REPORT")
        self.add_report_line("="*80)
        self.add_report_line("")
        self.add_report_line(f"Circuit:           ANTONIO")
        self.add_report_line("Technology:        45nm CMOS")
        self.add_report_line("Supply Voltage:    1.2V")
        self.add_report_line("Temperature:       27°C")
        self.add_report_line(f"Architecture:      1×OR2 + 1×AOI21 + 1×INV + 2×NAND2 + 2×XNOR2 + 1×OAI21")
        self.add_report_line(f"Netlist:           {self.netlist_file if self.netlist_file else 'antonio.sp'}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='PPA Analysis Tool - Antonio 4-2 Compressor',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
Usage Examples:
  # Use default area values (measurement file only)
  python3 ppa_analyzer_antonio.py -m antonio.sp.mt0
  
  # Extract actual area from netlist (recommended)
  python3 ppa_analyzer_antonio.py -m antonio.sp.mt0 -n antonio.sp
  
  # Specify output filename
  python3 ppa_analyzer_antonio.py -m antonio.sp.mt0 -n antonio.sp -o my_report
...
Important Notes:
  ...
  View reports:
    cat antonio_PPA_report.txt
    cat antonio_PPA_report.json | python -m json.tool
        '''
    )
    parser.add_argument(
        '-m', '--measure-file',
        default='antonio.sp.mt0',
        help='Xyce measurement output file (.mt0 file, default: antonio.sp.mt0)'
    )
    parser.add_argument(
        '-n', '--netlist',
    ...
    parser.add_argument(
        '-o', '--output',
        default='antonio_PPA_report',
        help='Output report base filename (default: antonio_PPA_report)'
    )
    ...
    # Friendly prompt
    print("="*80)
    print("Antonio PPA Analysis Tool")
    print("="*80)
    ...
    analyzer = PPAAnalyzerAntonio(args.measure_file, args.netlist)
