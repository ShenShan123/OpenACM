...
always @(*) begin
    if (D == 0) begin
        ...
        shift_ad = (lz_ad >= 5) ? (lz_ad - 5) : 5'd0;
        n_ad = (D >> shift_ad) & 5'h1F;
        m_ad = A[4:0];
        sum_ad = m_ad + n_ad;
        approx_ad = (6'h20 | sum_ad[4:0]) << (sum_ad[5] ? 6 : 5);
    end
end
...
always @(*) begin
    if (C == 0) begin
        ...
        shift_bc = (lz_bc >= 5) ? (lz_bc - 5) : 5'd0;
        n_bc = (C >> shift_bc) & 5'h1F;
        m_bc = B[4:0];
        sum_bc = m_bc + n_bc;
         approx_bc = (6'h20 | sum_bc[4:0]) << (sum_bc[5] ? 6 : 5);
    end
end

// ---------- 累加 ----------
wire [47:0] total_sum =  (approx_ad << (shift_ad + 18) )+ (approx_bc << (shift_bc + 18))+(P_AB << 36);
