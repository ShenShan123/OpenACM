...
always @(*) begin
    if (D == 0) begin
        ...
        shift_ad = (lz_ad >= 6) ? (lz_ad - 6) : 5'd0;
        n_ad = (D >> shift_ad) & 6'h3F;
        m_ad = A[5:0];
        sum_ad = m_ad + n_ad;
        approx_ad = (7'h40 | sum_ad[5:0]) << (sum_ad[6] ? 7 : 6);
    end
end
...
always @(*) begin
    if (C == 0) begin
        ...
        shift_bc = (lz_bc >= 6) ? (lz_bc - 6) : 5'd0;
        n_bc = (C >> shift_bc) & 6'h3F;
        m_bc = B[5:0];
        sum_bc = m_bc + n_bc;
        approx_bc = (7'h40 | sum_bc[5:0]) << (sum_bc[6] ? 7 : 6);
    end
end

// ---------- 累加 ----------
wire [47:0] total_sum =  (approx_ad << (shift_ad + 17) )+ (approx_bc << (shift_bc + 17))+(P_AB << 34);
